...
LDA_FILE = "lda_topics.xlsx"
TOPIC_COL = "Topic"
KEYWORDS_COL = "Keywords"
DESC_COL = "Semantic_Description"

# 是否重置
RESET_THEME_TABLE = False
RESET_POST_THEME = False

# 风险分类时只处理 risk_id 为空的主题
ONLY_CLASSIFY_NULL_RISK = True
...
def is_bad_theme_output(text: str) -> bool:
    """
    过滤不合格输出：
    - 纯数字
    - 第N个/主题N/编号N 这类编号式输出
    ...
    bad_patterns = ["第", "主题", "编号"]
    if any(p in t for p in bad_patterns) and any(ch.isdigit() for ch in t):
        return True
...
def ensure_theme_table():
    ...
    try:
        with conn.cursor() as cursor:
            sql = """
            CREATE TABLE IF NOT EXISTS theme_lda (
...
def reset_theme_table():
    ...
    try:
        with conn.cursor() as cursor:
            cursor.execute("TRUNCATE TABLE theme_lda")
        conn.commit()
    finally:
        conn.close()


def reset_posts_lda_theme():
    conn = get_connection()
    try:
        with conn.cursor() as cursor:
            cursor.execute("UPDATE posts SET lda_theme = NULL")
        conn.commit()
    finally:
        conn.close()
...
# 6. LDA 主题解释（写回 Excel）
# =========================
def interpret_lda_topic(topic_id, keywords):
    prompt = f"""
你是一个擅长主题分析的专家。

下面是一组由LDA主题模型输出的主题关键词，其中每个词前面的数值表示该词在该主题中的重要程度（权重）：

主题编号：{topic_id}
主题关键词：{keywords}

请根据这些带权重的关键词，生成一个语义清晰、可读性强、适合论文展示的中文主题短句。

要求：
1. 充分参考关键词权重，优先围绕高权重词概括主题
2. 输出一句中文短句即可
3. 长度控制在10~20字左右
4. 表达自然、准确、概括性强
5. 不要直接重复粘贴关键词
...
"""
    ...
    response = client.chat.completions.create(
        model=MODEL_NAME,
        messages=[
            {"role": "system", "content": "你是一个擅长从LDA关键词中提炼语义主题的专家。"},
...
def ensure_lda_excel_has_desc():
    print(f"检查 {LDA_FILE} 是否已有 {DESC_COL} ...")
    df = pd.read_excel(LDA_FILE)

    if TOPIC_COL not in df.columns or KEYWORDS_COL not in df.columns:
        raise ValueError(
            f"{LDA_FILE} 中缺少列 {TOPIC_COL} 或 {KEYWORDS_COL}，当前列：{list(df.columns)}"
        )
    ...
    for idx, row in df.iterrows():
        ...
        print(f"[LDA解释] 正在处理第 {idx + 1}/{len(df)} 个主题...")
        print(f"Topic: {topic_id}")
        print(f"Keywords: {keywords}")

        try:
            desc = interpret_lda_topic(topic_id, keywords)
            print(f"生成结果：{desc}")
        ...
        df.to_excel(LDA_FILE, index=False)
        time.sleep(SLEEP_SECONDS)

    if changed:
        print(f"{LDA_FILE} 已补充/更新 {DESC_COL}")
    else:
        print(f"{LDA_FILE} 已有完整 {DESC_COL}，跳过解释。")
...
def insert_theme(theme, theme_source):
    ...
    try:
        with conn.cursor() as cursor:
            sql = """
                INSERT IGNORE INTO theme_lda (theme, theme_source)
                VALUES (%s, %s)
            """
...
def load_topics_from_excel():
    ...
    if DESC_COL not in df.columns:
        raise ValueError(f"{LDA_FILE} 中没有找到列 {DESC_COL}")
...
def seed_theme_table():
    print("开始把 Excel 中已有 LDA 主题写入 theme_lda...")
    topics = load_topics_from_excel()

    for topic in topics:
        insert_theme(topic, "existing")

    print(f"LDA 主题导入：{len(topics)} 条")
...
1. 必须返回“主题原文”，不能返回编号
2. 不要返回数字（如 1、2、3）
3. 如果候选主题中有一个能准确概括帖子核心内容，就直接返回该主题原文
4. 如果都不合适，返回：NONE
5. 不要解释，不要多写任何内容
...
"""
    ...
    if is_bad_theme_output(result):
        print(f"⚠️ 模型返回了无效主题结果：{result}")
        return None
    ...
    if is_bad_theme_output(result2):
        return None
...
当前帖子无法匹配已有LDA主题库，请根据以下信息生成一个新的综合主题短句。
...
"""
    ...
    if is_bad_theme_output(result):
        raise ValueError(f"生成的新主题无效：{result}")
...
# 14. 回写 posts
# =========================
def update_post_theme(post_id, lda_theme=None):
    conn = get_connection()
    try:
        with conn.cursor() as cursor:
            sql = """
                ...
                WHERE post_id = %s
            """
            cursor.execute(sql, (lda_theme, post_id))
        conn.commit()
    finally:
        conn.close()
...
def main():
    ...
    if RESET_POST_THEME:
        print("清空 posts.lda_theme ...")
        reset_posts_lda_theme()
    ...
    print(f"\n当前数据库中的 LDA 候选主题数：{len(candidate_topics)}")
    ...
    for idx, (post_id, title, content) in enumerate(posts, start=1):
        ...
        try:
            ...
            lda_theme = match_existing_theme(
                title=title,
                content=content,
            ...
            if lda_theme:
                matched_count += 1
                print(f"匹配到 LDA 主题：{lda_theme}")
                insert_theme(lda_theme, "existing")
            else:
                ...
                generated_count += 1
                print(f"新生成 LDA 主题：{lda_theme}")
                insert_theme(lda_theme, "generated")
                if lda_theme not in candidate_topics:
                    candidate_topics.append(lda_theme)

            update_post_theme(post_id, lda_theme)
            print("已回写到 posts.lda_theme")
            success_count += 1
